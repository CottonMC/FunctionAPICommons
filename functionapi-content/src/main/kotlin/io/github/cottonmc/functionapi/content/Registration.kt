...
class Registration(private val subfolder: String,
    ...
    internal var context = PermanentHashmap<String, Any>()
    val staticCommandExecutor = StaticCommandExecutor()

    fun register(contentRegistrations: List<ContentRegistration>): CommandDispatcher<MutableMap<String, Any>> {
    ...
    private fun generateIdsAndRegister(contentRegistrations: List<ContentRegistration>, context: Map<String, Any>) {
        ...
        var postfix = (registrationContainer.getPostfixes()).stream().map{it.trim()}.collect(Collectors.joining("_"))
        if (!postfix.isEmpty()) {
            postfix = postfix.removePrefix("_")
        }
